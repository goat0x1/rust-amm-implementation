...
impl AmmTestHarnessProgramTest {
    ...
    pub async fn assert_quote_matches_simulated_swap(
        ...
        let swap_params = SwapParams {
            ...
            missing_dynamic_accounts_as_default: true,
        };
...
pub async fn take_snapshot(
    ...
    let mut _dex_wrapper_mints = HashSet::new();
    let mut amm = amm_factory(&keyed_account, &mut _dex_wrapper_mints)?;
    let amm: &mut (dyn Amm + Send + Sync) = amm.as_mut();

    for _ in 0..3 {
        test_harness.update_amm(amm);
    }
    println!("Updating AMM from snapshot...");
